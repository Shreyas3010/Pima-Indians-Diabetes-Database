...
row1=np.arange(144)
results= pd.DataFrame(data=None,index=row1,columns = ['ratio SMOTE','ratio RUS','Class','Datasize','Training Datasize','After sampling SMOTE','After sampling RUS','Precision','Recall','f1-score','oobscore','AUC','Testing Datasize'])
#0.6 is limit
A=[0.9,0.8,0.7]    
B=[0.9,0.8,0.7,0.6]
a=0
...
for i in A:
    ...
    for j in B:
        ...
        if rat2 <= 1:
            rus=RandomUnderSampler(sampling_strategy=rat2,random_state=5)
            X_train_sampled,y_train_sampled  = rus.fit_sample(X_train_sampled1,y_train_sampled1)
            samplingdatasize=collections.Counter(y_train_sampled)
            print("sampled training data size RUS",samplingdatasize)
            results['After sampling RUS'][b]=samplingdatasize[0]
            results['After sampling RUS'][a]=samplingdatasize[1]
            ...
            i1=str(i)
            j1=str(j)
            plt.title('Receiver operating characteristic example SMOTE'+i1+'RUS'+j1)
            plt.legend(loc="lower right")
            plt.savefig('Graph/SMOTE'+i1+'RUS'+j1+'.png')
            plt.show()
        a=a+1
        print("a",a)
