...
row1=np.arange(54)
results= pd.DataFrame(data=None,index=row1,columns = ['ratio SMOTE','ratio Near Miss 01','Class','Datasize','Training Datasize','After sampling SMOTE','After sampling Near Miss 01','Precision','Recall','f1-score','oobscore','AUC','Testing Datasize'])
#0.6 is limit
A=[0.9,0.8,0.7]    
...
for i in A:
    ...
    for j in B:
        ...
        print("ratio Near Miss 01",j)
        results['ratio Near Miss 01'][a]=j
        b=a
        a=a+1
        results['Class'][b]=0
        results['Class'][a]=1
        ...
        print("sampling_strategy Near Miss 01",rat2)
        ...
        if rat2 <= 1:
            nm = NearMiss(sampling_strategy=rat2,version=1,random_state=5,n_neighbors=3)
            ...
            print("sampled training data size Near Miss 01",samplingdatasize)
            results['After sampling Near Miss 01'][b]=samplingdatasize[0]
            results['After sampling Near Miss 01'][a]=samplingdatasize[1]
            ...
            i1=str(i)
            j1=str(j)
            plt.title('Receiver operating characteristic example SMOTE'+i1+'Near Miss 01 '+j1)
            plt.legend(loc="lower right")
            plt.savefig('Graph/SMOTE'+i1+'NearMiss01_'+j1+'.png')
            plt.show()
        a=a+1
        print("a",a)
